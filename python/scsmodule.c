...
static PyObject *csolve(PyObject *self, PyObject *args, PyObject *kwargs) {
    ...
    Sol sol = {0};
    Info info;
    char *kwlist[] = {"shape", "Ax",      "b",         "c",         "cone",
                      "warm",  "verbose", "normalize", "max_iters", "scale",
                      "eps",   "cg_rate", "alpha",     "rho_x",     SCS_NULL};
/* parse the arguments and ensure they are the correct type */
    ...
    if (!PyArg_ParseTupleAndKeywords(
            ...
            &PyArray_Type, &Ax, &PyArray_Type, &b, &PyArray_Type, &c,
            &PyDict_Type, &cone, &PyDict_Type, &warm, &PyBool_Type, &verbose,
            &PyBool_Type, &normalize, &(d->stgs->max_iters), &(d->stgs->scale),
    ...
    /* set A */
    if (!PyArray_ISFLOAT(Ax) || PyArray_DIM(Ax, 1) != d->m ||
        PyArray_DIM(Ax, 0) != d->n) {
        return finishWithErr(d, k, &ps, "Ax must be a numpy array of floats of "
                                        "dim m times n (is this a row-major vs "
                                        "col-major order issue?)");
    }
