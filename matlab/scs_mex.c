...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	...
	if (ALPHA_mex == NULL)
		...
		d->ALPHA = (pfloat) *mxGetPr(ALPHA_mex);

	const mxArray *RHO_X_mex = mxGetField(params, 0, "RHO_X");
	if (RHO_X_mex == NULL)
		d->RHO_X = 1e-3;
	else
		d->RHO_X = (pfloat) *mxGetPr(RHO_X_mex);

	const mxArray *MAX_ITERS_mex = mxGetField(params, 0, "MAX_ITERS");
	if (MAX_ITERS_mex == NULL)
		d->MAX_ITERS = 2500;
	else
		d->MAX_ITERS = (idxint) *mxGetPr(MAX_ITERS_mex);

	const mxArray *SCALE_mex = mxGetField(params, 0, "SCALE");
	if (SCALE_mex == NULL)
		d->SCALE = 1;
	else
		d->SCALE = (pfloat) *mxGetPr(SCALE_mex);
	...
	if (VERBOSE_mex == NULL)
		...
		d->VERBOSE = (idxint) *mxGetPr(VERBOSE_mex);

	const mxArray *NORMALIZE_mex = mxGetField(params, 0, "NORMALIZE");
	if (NORMALIZE_mex == NULL)
		d->NORMALIZE = 1;
	else
		d->NORMALIZE = (idxint) *mxGetPr(NORMALIZE_mex);
	...
	if (ep)
		k->ep = (idxint) *mxGetPr(ep);
	else
		k->ep = 0;

	mxArray * ed = mxGetField(cone, 0, "ed");
	if (ed)
		k->ed = (idxint) *mxGetPr(ed);
	else
		k->ed = 0;
