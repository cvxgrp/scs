...
if (flags.COMPILE_WITH_OPENMP)
    cmd = sprintf('mex -O %s COMPFLAGS="/openmp \\$COMPFLAGS" CFLAGS="\\$CFLAGS -O3 -fopenmp -DOPENMP -DMATLAB_MEX_FILE %s %s" ../linsys/indirect/private.c %s -I../include %s %s %s %s -output scs_indirect',  arr, d, flags.LCFLAG, common_scs, flags.INCS, link, flags.LOCS, flags.BLASLIB);
else
