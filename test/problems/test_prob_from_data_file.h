...
/* Solve `file` at the given tolerance and check the objective against OPT to
 * within `obj_tol`. Most problems want the defaults of
 * `_test_prob_from_data` below; a problem with a slow tail (see issue_220)
 * needs a looser tolerance to converge in a sane number of iterations. */
static const char *_test_prob_from_data_eps(const char *file, scs_float OPT,
                                            scs_float eps, scs_float obj_tol) {
  scs_int read_status;
  ScsData *d;
  ScsCone *k;
  ScsSettings *stgs;
  ...
  success = ABS(perr) < obj_tol && ABS(derr) < obj_tol &&
            exitflag == SCS_SOLVED;
  if (!success) {
    scs_printf("%s: FAILED\n", file);
  }
...
static const char *_test_prob_from_data(const char *file, scs_float OPT) {
  return _test_prob_from_data_eps(file, OPT, 1e-6, 1e-4);
}
