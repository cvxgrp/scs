function [x,y,s,info] = scs_acc(data,K,params)
% cone solver, solves:
%
% min. c'x
% subject to Ax + s = b
...
for i=0:max_iters-1
    ...
    if false
        ...
        if uv_prev_prev ~= -1
            yy = -[u;v] + uv_prev + phi_prev - uv_prev_prev;
            %yy = -phi_prev + uv_prev_prev + phi_prev_prev - uv_prev_prev_prev;
            %yy = uv_prev - uv_prev_prev;
            B = B + (yy - B * ss) * ss' / (ss'*ss);
        end
        %phi_prev_prev = phi_prev;
        phi_prev = [u;v];
        ss = - B \ (uv_prev - [u;v]);
        uv = uv_prev + ss;
        u = uv(1:n+m+1);
