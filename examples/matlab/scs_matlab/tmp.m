...
cvx_solver scs_acc
%cvx_solver('scs_matlab')
cvx_solver_settings('anderson_lookback', 20, 'extra_verbose', 0, 'eps',1e-9, 'scale', 1, 'normalize', 1, 'use_indirect', use_indirect, 'gen_plots', 0, 'max_iters', 2500)
variable x(n)
minimize(norm(A*x - b) + lam*norm(x,1))
x >= 0
cvx_end
...
variable x(n)
cvx_solver('scs')
cvx_solver_settings('extra_verbose', 0, 'eps',1e-9, 'scale', 1, 'normalize', 1, 'use_indirect', use_indirect, 'gen_plots', 1, 'max_iters', 2500)
minimize(norm(A*x - b) + lam*norm(x,1))
x >= 0
cvx_end
