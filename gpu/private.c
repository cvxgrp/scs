...
Priv * initPriv(const AMatrix * A, const Settings * stgs) {
   ...
   /* Get handle to the CUBLAS context */
    p->cublasStatus = cublasCreate(&cublasHandle);

    checkCudaErrors(cublasStatus);

    /* Get handle to the CUSPARSE context */
    p->cusparseStatus = cusparseCreate(&cusparseHandle);

    checkCudaErrors(cusparseStatus);

    cusparseMatDescr_t descr = 0;
    p->cusparseStatus = cusparseCreateMatDescr(&descr);
    ...
    cudaMalloc((void **)&d_col, nz*sizeof(scs_int));
    cudaMalloc((void **)&d_row, (N+1)*sizeof(scs_int));
    cudaMalloc((void **)&d_val, nz*sizeof(scs_float));
    cudaMalloc((void **)&d_x, N*sizeof(scs_float));
    cudaMalloc((void **)&d_r, N*sizeof(scs_float));
    cudaMalloc((void **)&d_p, N*sizeof(scs_float));
    cudaMalloc((void **)&d_Ax, N*sizeof(scs_float));

    cudaMemcpy(d_col, J, nz*sizeof(scs_int), cudaMemcpyHostToDevice);
    cudaMemcpy(d_row, I, (N+1)*sizeof(scs_int), cudaMemcpyHostToDevice);
    cudaMemcpy(d_val, val, nz*sizeof(scs_float), cudaMemcpyHostToDevice);
    CUDA_CHECK_ERR();

    XXX move to later:
    cudaMemcpy(d_x, x, N*sizeof(scs_float), cudaMemcpyHostToDevice);
    cudaMemcpy(d_r, rhs, N*sizeof(scs_float), cudaMemcpyHostToDevice);
