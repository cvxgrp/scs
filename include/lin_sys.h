...
/* forms y += A'*x */
void accum_by_atrans(const AMatrix *A, Priv *p, const scs_float *x, scs_float *y);
/* forms y += A*x */
 ...
 * called on output */
char *get_lin_sys_method(const AMatrix *A, const Settings *stgs);
...
void normalize_a(AMatrix *A, const Settings *stgs, const Cone *k, Scaling *scal);
