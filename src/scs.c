...
/* assumes length of array is n + m */
static scs_float dot_with_rho_x(ScsWork *w, const scs_float *x,
                                const scs_float *y) {
...
static scs_int update_work(const ScsData *d, ScsWork *w,
  ...
  w->root_plus_a = 1 + dot_with_rho_x(w, w->g, w->g);
  return 0;
}
