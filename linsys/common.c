...
void printAMatrix(AMatrix *A) {
    ...
    if (A->n * A->m < 2500) {
        ...
        for (i = 0; i < A->n; ++i) {
            ...
            for (j = 0; j < A->m; ++j) {
                scs_printf("A[%li,%li] = %4f, ", (long)j, (long)i,
                           A->x[i * A->m + j]);
            }
...
void normalizeA(AMatrix *A, const Settings *stgs, const Cone *k,
    ...
    scs_float minRowScale = MIN_SCALE * SQRTF(A->n),
              maxRowScale = MAX_SCALE * SQRTF(A->n);
    scs_float minColScale = MIN_SCALE * SQRTF(A->m),
              maxColScale = MAX_SCALE * SQRTF(A->m);
