...
ScsLinSysWork *scs_init_lin_sys_work(const ScsMatrix *A, const ScsMatrix *P,
  ...
  /* Create a non-default CUDA stream to avoid implicit default-stream
   * synchronization and improve GPU scheduling. */
  CUDA_CHECK_ABORT(
      cudaStreamCreateWithFlags(&p->stream, cudaStreamNonBlocking), p,
      "cudaStreamCreateWithFlags");

  /* Create cuDSS handle and bind to our stream */
  CUDSS_CHECK_ABORT(cudssCreate(&p->handle), p, "cudssCreate");
  CUDSS_CHECK_ABORT(cudssSetStream(p->handle, p->stream), p,
                    "cudssSetStream");
...
scs_int scs_solve_lin_sys(ScsLinSysWork *p, scs_float *b, const scs_float *ws,
  ...
  /* Copy RHS to pinned staging buffer */
  memcpy(p->h_b_pinned, b, vec_bytes);

  /* Pipeline: async H->D, solve, async D->H — all on the same stream.
   * Stream ordering guarantees each step completes before the next begins. */
  custatus = cudaMemcpyAsync(p->d_b, p->h_b_pinned, vec_bytes,
  ...
  custatus = cudaMemcpyAsync(p->h_sol_pinned, p->d_sol, vec_bytes,
  ...
  /* Single sync point — block until the D->H copy is complete */
  custatus = cudaStreamSynchronize(p->stream);
  if (custatus != cudaSuccess) {
    scs_printf("scs_solve_lin_sys: Error in stream sync: %d\n",
               (int)custatus);
    return custatus;
  }
